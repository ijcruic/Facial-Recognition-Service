...
def initialize_model(model_path, set_eval=True):
    if not os.path.exists(model_path):
        logger.error(f'This is broken - {model_path} not found')
        sys.exit(0)

    weights_dir = model_path if os.path.isdir(model_path) else os.path.dirname(model_path)
    detector_path = os.path.join(weights_dir, 'ATFaceDetect_v3.pth')
    ...
    cfg = {
        ...
        'multi_task': False,
        'visualize': True,
        'backbone_dir': weights_dir,
        'backbone_weights_filename': os.path.join(weights_dir, 'resnet50-11ad3fa6.pth'),
    }
    ...
    if not os.path.exists(detector_path):
        ...
        for alt in alt_paths:
            if os.path.exists(alt):
                detector_path = alt
                cfg['backbone_dir'] = os.path.dirname(detector_path)
                cfg['backbone_weights_filename'] = os.path.join(
                    cfg['backbone_dir'], 'resnet50-11ad3fa6.pth'
                )
                break
...
class RetinaFace(nn.Module):
    def __init__(self, cfg=None, phase='train', restore=False):
        ...
        if cfg['name'] == 'mobilenet0.25':
            ...
            if cfg['pretrain']:
                candidate_paths = []
                backbone_weights_filename = cfg.get('backbone_weights_filename')
                backbone_dir = cfg.get('backbone_dir')
                if backbone_weights_filename:
                    candidate_paths.append(backbone_weights_filename)
                if backbone_dir:
                    candidate_paths.append(os.path.join(backbone_dir, 'resnet50-11ad3fa6.pth'))
                # Fallbacks for historical layouts
                candidate_paths.extend([
                    'models_weights/recognition_at_face/resnet50-11ad3fa6.pth',
                    '../models_weights/recognition_at_face/resnet50-11ad3fa6.pth',
                    '/service/model_weights/recognition_at_face/resnet50-11ad3fa6.pth',
                ])

                for candidate in candidate_paths:
                    if candidate and os.path.exists(candidate):
                        state_dict = torch.load(candidate, map_location=torch.device('cpu'))
                ...
                if backbone is None:
                    ...
                    if weights_enum is not None:
                        backbone = models.resnet50(weights=weights_enum.DEFAULT)
                    else:
                        backbone = models.resnet50(weights="DEFAULT")

                    # Persist weights for offline reuse if possible
                    target_path = backbone_weights_filename or (
                        os.path.join(backbone_dir, 'resnet50-11ad3fa6.pth')
                        if backbone_dir else None
                    )
                    if target_path:
                        try:
                            os.makedirs(os.path.dirname(target_path), exist_ok=True)
                            torch.save(backbone.state_dict(), target_path)
                        except OSError as exc:
                            logger.warning(f"Unable to cache ResNet50 backbone weights to {target_path}: {exc}")
            else:
                backbone = models.resnet50(weights=None)
